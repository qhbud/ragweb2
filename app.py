...
def initialize_rag_system():
    """Initialize the RAG system with proper error handling and thread safety"""
...
Answer: """
        ...
        # Store in global variables (thread-safe assignment)
        with initialization_lock:
            rag_system = {
                'qa_chain': qa_chain,
...
class ProductionServer:
    def __init__(self, app, host='0.0.0.0', port=8000, threaded=True):
        self.app = app
        self.host = host
        self.port = port
    ...
    def start(self):
        ...
        try:
            # Use Werkzeug's make_server for better production handling
...
if __name__ == '__main__':
    ...
    if debug:
        ...
        # Production mode - use custom production server
        server = ProductionServer(app, host=host, port=port, threaded=True)
        server.start()
    else:
        # Fallback - use Flask's built-in server without debug
        logger.info(f"Starting Flask server on {host}:{port}")
        app.run(host=host, port=port, debug=False, threaded=True)
else:
    # For WSGI deployment (if needed)
    logger.info("Flask app loaded for WSGI deployment")
